...
@njit
def update_astrocyte_dynamics(dt, identifier, gamma, parents, DiffNa, DiffK, DiffCa, g_l, distance_from_soma, radius, area, volume, Cm, stim_start_nt, stim_end_nt, stim_dur, glut_stim, pot_stim, stim_comp_glut, stim_comp_pot, n_stim, n_stimulated, N, last, iskip):
    t_in = 20.0
    ...
    v = np.ones(N) * -83.79    # Membrane potential of the astrocyt(Ac) mV 
    vs = np.zeros((N, (last-int(t_in/dt))//iskip))    
    Ki = np.ones(N) * 146000.0    # K+ concentration in the astrocyte uM 
    Kis = np.zeros((N, (last-int(t_in/dt))//iskip))
    K_stim = np.ones(N)
    K_bath_BL = 2900.0
    K_bath = np.ones(N) * K_bath_BL
    Nai = np.ones(N) * 12000.0   # Na+ concentration in the astrocytes(uM) 
    # Nak_BL_threshold = 10 #um
    # Nak[distance_from_soma > Nak_BL_threshold] = 15000
    Nais = np.zeros((N, (last-int(t_in/dt))//iskip))
    HCO3i = np.ones(N) * 25000.0  # HCO3-concetration in the astrocytes(uM)
    Cai = np.ones(N) * 0.082    # The astrocytic cytosolic Ca2+ concentation(uM) 
    Cais = np.zeros((N, (last-int(t_in/dt))//iskip))
    Cli = np.ones(N) * 8215.0     # Cl- concentration in the astrocyts via electroneutrality(uM) 
    ...
    h_ip3 = np.ones(N) * 0.4086     # The inactivation variable of the astrocytic IP3R channel(-) 
    Ip3_i = np.ones(N) * 0.0483    # The astrocytic inositol triphophate(IP3) concentration(uM) 
    ...
    Ko = np.ones(N) * 2900.0    # K+ concentration in the synaptic cleft(sc)(uM)
    Kos = np.zeros((N, (last-int(t_in/dt))//iskip))
    Nao = np.ones(N) * 152000.0   # Na+ concentration in the SC(uM)
    Naos = np.zeros((N, (last-int(t_in/dt))//iskip))
    HCO3o = np.ones(N) * 26000.0  # HCO3-concentration in the SC(UM)
    pho = np.ones(N) * 7.35
    ...
    Cap = 1846.40  # Perivascular Calcium concentration uM    
    ...
    I_pump_max = 2.3667e4  # Maximum flux through the Na+/K+ ATP-ase pump uMs^-1
    # Conductances in uMmV^-1s^-1
    # original conductances in mhom^-2 converted by dividing by R_K=6d-8 and F=9.65d4
    ...
    glut_stim = np.ones(N) * 1300.00 # to have global, decaying stimulation at each compartment. THis replaces the one that comes from the UI
    ...
    # pot_stim = np.ones(N) * 4100.00   # -2300 4100.00# to have global, decaying stimulation at each compartment. THis replaces the one that comes from the UI
    pot_stim_amp = pot_stim  # 7000 600    
    alpha =  0.04 # 0.04 for test 50 sec, o/w 0.08
    ...
    for ii in range(1, last+1):
        ...
        for k in range(N):
            if glut_stim is not None:
                if stim_start_nt*dt <= ii*dt <= stim_end_nt*dt:
                    ...
                    for i, comp in enumerate(stim_comp_glut):
                        if k == comp:
                            stim_value = glut_stim[i]                            
                            Glu[k] = stim_value * np.exp(-(ii*dt - stim_end_nt*dt) / 0.5)
                        if N != 1:                                                 
                            j2 = parents[k]            
                            Icoup[k] = gamma[k] * (v[j2-1] - v[k])  ## uA/cm^2            
                            Icoup[j2-1] += gamma[j2-1] * (v[k] - v[j2-1])            
                ...
                if stim_start_nt*dt <= ii*dt <= stim_end_nt*dt:
                    ...
                    for i, comp in enumerate(stim_comp_pot):
                        if k == comp:
                            ...
                            K_stim[k] = stim_value * np.exp(-beta * (ii * dt - stim_end_nt  * dt)) #+ 2900 * (1 - np.exp(-beta * (ii * dt - stim_end_nt * dt))) 
            ...
            # overwrite constant volume and area logic above to implement compartment specific volume and area
            ...
            # Cl_concentration in the SC via electroneutrality(uM)
            Clo = Nao[k] + Ko[k] - HCO3o[k]          
         
            rho = rhomin + (rhomax - rhomin) / Glumax * Glu[k] # *0.3d-3 
            ...
            KKs_alpha2beta2 = y = 3.6 * 1000  # 2.37718e-07 * vk[k]**4 + 3.51700e-05 * vk[k]**3 + 1.97647e-03 * vk[k]**2 + 2.42484e-02 * vk[k] + 1.07469e+00 
            ...
            I_k_leak = 0.18 * g_k * (v[k] - EKk) # K+ flux through the K+ channel(uMs^-1)
            
            # if time_t >= 90.00:
            #     JNak[k] = JNak[k]  
            # else:
            I_Na_leak[k] = 1.3 * g_Na * (v[k] - ENak) #Na+ flux through the Na+ channel(uMs^-1) 2.6
            
            I_NBC[k] =  g_NBC * (v[k] - ENBCk) #Na+ and HCO3- flux through the NBC channel (uMs^-1)  
       
            I_KCC1 =  g_KCC1 * phii * np.log((Ko[k] * Clo) / (Ki[k] * Cli[k])) * 30.0  #Cl- and K+ flux through the KCC1 channel(uMs^-1) 
                   
            I_NKCC1[k] =  g_NKCC1 * phii * np.log((Nao[k] * Ko[k] * Clo**2.0) / (Nai[k] * Ki[k] * Cli[k]**2.0)) 
            
            I_Cl_leak = g_Cl * (v[k] - EClk) #Cl-flux through the cl- channel(uMs^-1)          

            Bcyt = 1.0 / (1.0 + Bkend + Kex * Bex / (Kex + Cai[k])**2.0) #Fast Ca2+ buffering is described with in the steady state approximation(-)
            
            G = (rho + delta) / (KG + rho + delta)   #The ratio of active to total G-protein due to metabotropic glutamate receptor 

            HCak  = Cai[k]/gamCaik + Cap/gamCaek 
            eta   = (R - R0passivek)/(R0passivek)    
            minfk = (1.0 / (1.0 + np.exp(-(eta - epshalfk) / Kappa))) * ((1.0 / (1.0 + HCak)) * (HCak + np.tanh((v[k] - v1TRPVk) / v2TRPVk)))  # The equilibrium state of the TRPV4 channel (-)
            ...
            KGluTmg = 34.0  # uM
            # KGlutHo = 0.1 # uM
            JGluT = IGluTmax * Ki[k] / (Ki[k] + KGluTmK) * Nao[k]**3.0 / (Nao[k]**3.0 + KGluTmN**3.0) * Glu[k] / (Glu[k] + KGluTmg) # test:* Ho[k] / (Ho[k] + KGlutHo)
            ...
            ENHE=26.7 * (np.log((Nai[k]*Ho[k])/(Nao[k]*Hi[k])))            
            ...
            # uncomment for multicompartmental model
            ...
            #     vk[k] = vk[k]
            # else:
            v[k] = v[k] + dt * (conv * (-I_k_leak-I_Cl_leak-I_Na_leak[k]-I_pump[k]+I_NBC[k] + Na_stim[k]- I_NHE[k] -2.0 * I_TRPV[k] + Isyn[k] * 0.01 * convg + Istim[k] * 0.01 * convg + Icoup[k] * 0.01 * convg - I_NCX[k]*convg + 2.0 * JGluT * convg)) #(-JBKk - JKk - JClk - JNBCk - JNak - JNaKk - 2.0 * JTRPVk - (-2.0 * JGluT + JNCX - JNMDA - Jampa) * convg)  #Istim[k] * 0.01 * convg ---- uA/cm^2 to pA/um^2  overall mV/s
           
            # To stop immediately and test if Nan is encountered   
            # if (np.isnan(v[k])): 
            #     print("Nan values encountered")  
            #     # print(ii, k)             
            #     break

            Ki[k] = Ki[k] + dt * (-I_k_leak + 2.0 * I_pump[k] + I_NKCC1[k] + I_KCC1 - JGluT * convg) #-JKk + 2.0 * JNaKk + JNKCC1k + JKCC1k - JBKk - JKIRk - (JGluT + JNMDAKk + JampaKk) * convg  # JNMDA * convg
            Nai[k] = Nai[k] + dt * (-I_Na_leak[k] - 3.0 * I_pump[k] + I_NBC[k] - I_NHE[k] + I_NKCC1[k] + IdiffNa[k] + Na_stim[k] * 1000.0 - 3.0 * I_NCX[k] * convg  + 3.0 * JGluT * convg) #-JNak - 3.0 * JNaKk + JNKCC1k + JNBCk - 3.0 * JNaCak - (3.0 * JNCX - JNMDANak - JampaNak - 3.0 * JGluT) * convg
            ...
            Cli[k] = Cli[k] + dt * ((-I_k_leak + 2.0 * I_pump[k] + I_NKCC1[k] + I_KCC1 - JGluT * convg) + (-I_Na_leak[k] - 3.0 * I_pump[k] + I_NBC[k] - I_NHE[k] + I_NKCC1[k] + IdiffNa[k] + Na_stim[k] * 1000.0 - 3 * I_NCX[k] * convg  + 3 * JGluT * convg) -(2 * I_NBC[k] + I_NHE[k]) + 2.0  * (Bcyt * (I_NCX[k] * convg + I_IP3[k] - Jpump + JERleak + IdiffCa[k] + I_TRPV[k]/rbuff))) #fun[2] + fun[1] - fun[3] + 2.0 * fun[4]
            
            Ko[k] = Ko[k] + dt * (1.0/VRsa * (I_k_leak - 2.0 * I_pump[k] - I_NKCC1[k] - I_KCC1 + JGluT * convg)  + IdiffK[k] + I_K_bath_to_Ko[k]) #  1.0 / VRsa * (JKk - 2.0 * JNaKk - JNKCC1k - JKCC1k + JKIRk + (JNMDAKk + JampaKk + JGluT) * convg) + JKNEtoSCk  # (JNMDAKk + JampaKk + JEAATKk) * convg
            Nao[k] = Nao[k] + dt * (1.0/VRsa * (I_Na_leak[k] + 3.0 * I_pump[k] - I_NBC[k] + I_NHE[k] - I_NKCC1[k] + 3.0 * I_NCX[k] * convg - 3.0 * JGluT * convg)) #  1.0 / VRsa * (JNak + 3.0 * JNaKk - JNKCC1k - JNBCk - (-3.0 * JNCX + JNMDANak + JampaNak + 3.0 * JGluT) * convg) + JNaNEtoSCk  # (JNMDANak + JampaNak + JEAATNak) * convg
        ...
        if time_t > t_in and ii % iskip == 0:
            ...
            # Glus[:, j_10] = Glu
            j_10 += 1  
    return vs,  Kos, Nais, Clis, Kis, Naos, time_array, I_pumps, phis, phos, Icoups, IdiffKs, IdiffNas, IdiffCas, Cais, I_Na_leaks, I_TRPVs, Ip3_is, I_NBCs, I_NHEs, I_NKCC1s, I_NCXs, I_pumps_alpha2beta1, I_pumps_alpha2beta2

 
def concat_ast_param(vs, Nak, Kos, Kks, Cak, pho, phi, IdiffKs, JNaks, JNaKks, Iks, time, JNBCks, JNHEs, JNKCC1ks, JNCXs, JNaKks_alpha2beta1, JNaKks_alpha2beta2, N1, N2):
    ...
    IdiffK = IdiffK/1000 #uM/s to mM/s
    ...
    df = pd.concat([time, vs, nak, cak, kos, kks, pho, phi, IdiffK, JNak, JNKA, JNKA_alpha2beta1, JNKA_alpha2beta2, Ik, JNBCk, JNHE, JNKCC1k, JNCX], axis=1)
    ...
    columns = ['Time'] + \
          ...
          [f'IdiffK_{i}' for i in range(N1,N2)] + \
